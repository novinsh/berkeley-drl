...
class Agent(object):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        v_t = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: ob_no})
        v_tp1 = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
        q = re_n + (1-terminal_n) * self.gamma*v_tp1 
        adv_n = q - v_t

        if self.normalize_advantages:
            adv_n = (adv_n - adv_n.mean()) / (adv_n.std()+eps) # YOUR_HW2 CODE_HERE
        return adv_n
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            v_tp1 = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no:next_ob_no})
            y = re_n + (1-terminal_n) * self.gamma * v_tp1
